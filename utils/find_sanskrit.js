...
processPages(
  (url, rel, text) => {
    ...
    let item = $("em.sanskrit");
    if (item.length > 0) {
      item.each((i, item) => {
        const t = $(item);
        const content = t.text().toLocaleLowerCase();
        sanskrit.ensure(content);
        sanskrit.dict.get(content)?.items.push({ file: rel, url: `${url}#${t.parents(".sentence").attr("id")}` });
  ...
  .then((_) => {
    console.log();
    console.log("done");
    fs.writeFileSync("terms.txt", Bun.inspect(sanskrit.dict.entries()));
  })
  .catch((err) => {
    console.log();
    console.log(url);
