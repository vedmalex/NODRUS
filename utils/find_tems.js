...
processPages(
  (url, rel, text) => {
    ...
    if (item.length > 0) {
      item.each((i, item) => {
        ...
        dict.ensure(content);
        dict.dict.get(content)?.items.push({ file: rel, url: `${url}#${t.parents(".sentence").attr("id")}` });
        t.attr("id", `${dict.ids[content]}`);
      });
      return $.html();
    } else {
      return text;
  ...
  .then((_) => {
    console.log();
    console.log("done");
    fs.writeFileSync("terms.txt", Bun.inspect(dict.dict.entries()));
  })
  .catch((err) => {
    console.log();
    console.log(url);
